...
namespace DivEditor.Controls
{
    internal static class FileManager
    {
        ...
        public static bool ReadConfig(string inpFile)
        {
            if (File.Exists(inpFile))
            {
                ...
                if (lines.Length >= 8)
                {
                    ...
                    // Загружаем последние координаты пользователя, если они есть
                    if (lines.Length >= 12)
                    {
                        try
                        {
                            GameData.lastUserTileBiasX = int.Parse(lines[9]);
                            GameData.lastUserTileBiasY = int.Parse(lines[11]);
                        }
                        catch
                        {
        ...
        public static void WriteConfig()
        {
            ...
            lines[8] = "// Last user coordinates X";
            lines[9] = GameData.lastUserTileBiasX.ToString();
            lines[10] = "// Last user coordinates Y";
            lines[11] = GameData.lastUserTileBiasY.ToString();
            File.WriteAllLines("Editor\\config.ini", lines);
        }
        //------------------------------------------------------------------------------------------------------------------------
        public static void SaveUserCoordinates()
        {
            // Обновляем координаты в GameData и сохраняем конфиг
            GameData.lastUserTileBiasX = Editor.Controls.MGGraphicalOutput.tileBiasX;
            GameData.lastUserTileBiasY = Editor.Controls.MGGraphicalOutput.tileBiasY;
            WriteConfig();
        }
        //------------------------------------------------------------------------------------------------------------------------
